...
disclaimer = '''
-----
This is a PyTorch implementation of the SonoNet model proposed in [1]. 
The model (eventually pretrained on FetalPlanesDB) is trained & tested on US liver images for standard-plane 
classification.

[1] Baumgartner et al. "Real-Time Detection and Localisation of Fetal Standard
...
def main():
    ...
    if args.sampler:
        ...
        if not os.path.exists(os.path.join(args.data_dir, f"sampler_weight_seed_{str(args.seed)}.npy")):
            ...
            np.save(os.path.join(args.data_dir, f"sampler_weight_seed_{str(args.seed)}.npy"), samples_weight)
        else:
            samples_weight = np.load(os.path.join(args.data_dir,f"sampler_weight_seed_{str(args.seed)}.npy"))
